...
export const generatePieValuesForBreakdownLastXDays = (
    ...
    taskListMap.forEach((tasks, tasklist) => {
        ...
        const completedTasks = tasks.filter(
            (task) => task.completed !== undefined && new Date(task.completed).getTime() >= lastXDays.getTime()
        );
...
export const generateChartValuesLastXDays = (taskListMap: Map<string, Task[]>, days: number): ChartValueItem[] => {
    ...
    taskListMap.forEach((tasks) => {
        ...
        // Compare the epoch of the completion date vs last 30 days epoch.
        const completedTasks = tasks.filter(
            (task) => task.completed !== undefined && new Date(task.completed).getTime() >= last7Days.getTime()
        );
...
export default function Stats(): JSX.Element {
    ...
    return (
        <div className={"stats"}>
            Stats
            <h4>Breakdown of completed tasks by task list</h4>
            ...
            <Box>
                <h4>Task completion over time, last 7 days</h4>
